...
[folder, name, ext] = fileparts(epiPathVtk);
mesh2vtkWriter_f(mesh_epi,folder,'epi_fibers.vtk');
[folder, name, ext] = fileparts(epiPathVtk);
mesh2vtkWriter_f(mesh_endo,folder,'endo_fibers.vtk');
...
function mesh2vtkWriter_f(mesh,fullPath,fileName)
    ...
    %mesh to struct
    struct=mesh2structWriter_f2(mesh);
...
function struct = mesh2structWriter_f2(mesh)
    %% Dataset %%
    %save "dataset"
    struct.Dataset=mesh.dataset;
    %% Points %%
    %Check if there are point in "mesh"
    if(~isempty(mesh.points))
        
        %save "NumPoints"
        struct.NumPoints=size(mesh.points,1); % height(mesh.points);
        
        %save "points"
        struct.Points=mesh.points(:,2:4);
        
    else
        %save "points" as an empty matrix
        struct.Points=[];
    end
    
    %% Cells %%
    %Check if there are cells in "mesh"
    if(~isempty(mesh.cells))
        
        %save "NumCells"
        struct.NumCells=size(mesh.cells,1); %height(mesh.cells);
        
        %save "cell_definition" compensating the Ids offset of 1
        cell_definition=mesh.cells(:,2:mesh.cell_type_vert+1)-1;
        
        %define "Cells" as a cell matrix of 2 columns and "num_cells"
        %columns
        struct.Cells=cell(mesh.num_cells,2);
        
        %all elements in the first column are equal to the vertices in each
        %cell
        struct.Cells(:,1)={mesh.cell_type_vert};
        
        %the second column equals the ids of the vertices defining each
        %cell as a column vector
        struct.Cells(:,2)=mat2cell(cell_definition',size(cell_definition,2),ones(size(cell_definition,1),1))';
        
    else
        %save "cells" as an empty matrix
        struct.Cells=[];
    end
    
    %% Point Data %%
    %Check if there is "point_data" in "mesh"
    if ~isempty(mesh.point_data_text)
        
        %for each field in "point_data_text"
        for i=1:size(mesh.point_data_text,1)
   
            %save data Type
            struct.PointData(i).Type=char(mesh.point_data_text(i,1));
            
            %save data Name
            struct.PointData(i).Name=char(mesh.point_data_text(i,2));
            
            %save data Format
            struct.PointData(i).Format=char(mesh.point_data_text(i,3));
            
            %save Data
            struct.PointData(i).Data=mesh.points(:,(i-1)*10+1:(i-1)*10+mesh.point_data_width(i));
            
        end
    else
        %save "PointData" as an empty field
        struct.PointData=[];
    end
    
    %% Cell Data %%
    %Check if there is "cell_data" in "mesh"
    if ~isempty(mesh.cell_data_text)
        
        %for each field in "cell_data_text"
        for i=1:size(mesh.cell_data_text,1)
   
            %save data Type
            struct.CellData(i).Type=char(mesh.cell_data_text(i,1));
            
            %save data Name
            struct.CellData(i).Name=char(mesh.cell_data_text(i,2));
            
            %save data Format
            struct.CellData(i).Format=char(mesh.cell_data_text(i,3));
            
            %save Data
            struct.CellData(i).Data=mesh.cells(:,(i-1)*10+1:(i-1)*10+mesh.cell_data_width(i));
            
        end
        
    else
        %save "CellData" as an empty field
        struct.CellData=[];
    end
    
    %% Field %%
    %save "Field" as an empty field
    struct.Field=[];
    
    %% Cell Type %%
    %check if "cell_type_edges" is empty
    if ~isempty(mesh.cell_type_edges) %if it is not
        
        %save "cell_type_edges" as "CellTypes"
        struct.CellTypes=mesh.cell_type_edges;
